...
if __name__=="__main__":
    ...
    batch_size = 256
    model = transformer_nn(key_size=data_x.shape[1], query_size=data_x.shape[1], value_size=data_x.shape[1], num_hiddens=10, dropout=0.5)
    ...
    # SMD Machine 1.1 Masked
    step = 4
    masked_initial = torch.zeros([1, step, data_x.shape[1]]).to(device)
    for i in range(step):
        if i<2:
            masked_initial[0, i, i * 9:(i + 1) * 9] = 1
        if i>=2:
            masked_initial[0, i, i * 10:(i + 1) * 10] = 1
